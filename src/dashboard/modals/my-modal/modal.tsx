...
const btn = {
  primary: (disabled?: boolean): React.CSSProperties => ({
    ...
    background: disabled ? "#93C5FD" : tokens.blue,
    color: "#fff",
    border: "none",
    borderRadius: "8px",
    padding: "9px 18px",
...
const Modal: FC = () => {
        ...
        try {
          ...
          if (data.termsOfCondition) {
            try {
              ...
              console.error("Terms JSON parse error:", e);
            }
          }
        } catch (err) {
  ...
  const handleUpdateAddon = (i: number, v: string) => {
    const a = [...addonsArray];
    a[i] = v;
    setAddonsArray(a);
  };
  const handleRemoveAddon = (i: number) =>
    setAddonsArray(addonsArray.filter((_, idx) => idx !== i));

  // ── Save main ─────────────────────────────────────────────
  const handleSave = async () => {
    setSaving(true);
    try {
      const finalAddonsString = addonsArray
        .filter((s) => s.trim() !== "")
        .join(" + ");
      const dataToSave = {
        ...proposal,
      ...
      const existingResults = await items
        .query("UpdatedProposals")
        .eq("quoteId", dataToSave.quoteId)
        .find();
      if (existingResults.items.length > 0) {
        await items.update("UpdatedProposals", {
          ...dataToSave,
        ...
        >
          ...
          >
              ...
              >
                <div>
                  ...
                  {termsData?.terms?.length > 0 && (
                    ...
                    >
                      {termsData.terms.length} clause
                      {termsData.terms.length !== 1 ? "s" : ""}
                    </div>
                  )}
                </div>
                <div
              ...
              !termsEditMode ? (
                ...
                >
                  ...
                  <div style={{ fontSize: "13px", marginTop: "4px" }}>
                    Click "Edit Terms" to add contract clauses.
                  </div>
                </div>
              ) : (
                (termsEditMode ? editedTerms?.terms : termsData?.terms)?.map(
                  (term: any, idx: number) => (
                    <div
                      key={idx}
                    ...
                    >
                      ...
                      >
                        ...
                        {termsEditMode && (
                          <button
                            onClick={() => {
                              const updated = editedTerms.terms.filter(
            ...
            {termsEditMode && (
              ...
              >
                <button onClick={handleTermsCancel} style={btn.secondary()}>
                  Cancel
                </button>
                <button
                  onClick={handleTermsSave}
                ...
                <Button
                  ...
                  onClick={(e) => {
                    ...
                    dashboard.openModal({
                      ...
                      params: { proposalId: proposal.quoteId },
                    });
                  }}
                >
                  Preview
                      ...
                      <Layout gap="20px">
                        ...
                        <Cell span={12}>
                          ...
                          {editMode ? (
                            <Input
                              size="large"
                              value={editedProposal.eventType}
                              onChange={(e) =>
                                setEditedProposal({
                                  ...editedProposal,
                        ...
                        <Cell span={6}>
                          ...
                          {editMode ? (
                            <Input
                              size="large"
                              value={editedProposal.eventDate}
                              onChange={(e) =>
                                setEditedProposal({
                                  ...editedProposal,
                            ...
                            >
                              ...
                              <Box flexGrow={1} style={{ width: "100%" }}>
                                <Input
                                  ...
                                  value={addon}
                                  onChange={(e) =>
                                    handleUpdateAddon(idx, e.target.value)
                                  }
                  ...
                  >
                    ...
                    >
                      ...
                      {editMode ? (
                        ...
                        >
                          {formatCurrency(
                            proposal.totalQuote || proposal.total,
                          )}
                        </div>
                      )}
                    </div>
